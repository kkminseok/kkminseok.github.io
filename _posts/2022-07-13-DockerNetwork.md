...
호스트의 네트워크 인터페이스 카드를 가상화해 물리 네트워크 환경을 컨테이너에 동일하게 제공하는 기술이 있습니다.

따라서 컨테이너는 물리 네트워크상에서 가상의 맥(MAC)주소를 가집니다.

이렇게 연결된 컨테이너들은 기본적으로 네트워크 장비의 IP를 할당받기 때문입니다.

실제 테스트에는 물리적인 장비가 필요하기에 여기까지만 적겠습니다.
